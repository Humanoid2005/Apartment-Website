            ...
            <div className="bills-section">
                {bills?(bills.filter((item)=>{
                    ...
                    return (<div key={index} className="bill-item"  style={{backgroundColor:boxcolor}}>
                            ...
                            <PaymentEditFrame url={"https://apartment-website-production.up.railway.app/api/admin/payments/"+`${bill.house_number}`+`${bill.bill_number}`} title="Type: " valueName="type" data={bill.type} bgcolor={boxcolor} headingcolor={headingcolor}/>
                            ...
                            <PaymentEditFrame url={"https://apartment-website-production.up.railway.app/api/admin/payments/"+`${bill.house_number}`+`${bill.bill_number}`} title="Payment Deadline: " valueName="deadline" data={formatted_date} bgcolor={boxcolor} isDate={true} headingcolor={headingcolor}/>
                            {bill.pending?<PaymentEditFrame url={"https://apartment-website-production.up.railway.app/api/admin/payments/"+`${bill.house_number}`+`${bill.bill_number}`} title="Payment Amount: " valueName="amount" data={bill.amount} bgcolor={boxcolor} headingcolor={headingcolor}/>:<h3 style={{backgroundColor:boxcolor}}><span className="payment-heading" style={{backgroundColor:boxcolor}}>Payment Amount:</span> {bill.amount}</h3>}
                            {bill.pending?null:<form className="view-bill" style={{backgroundColor:boxcolor}} action={`https://apartment-website-production.up.railway.app/api/generate-receipt/${bill.house_number}${bill.bill_number}`} method="POST">
